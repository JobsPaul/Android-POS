...
import com.example.posapp.Ui.Login.Models.ICallBackUserToken;
...
public class ApiCallers {
    ...
    public void getEmployeesByUsername(String token, String username, final ICallBackUserProfile callback) {
        StringRequest request = new StringRequest(Request.Method.GET, URLs.ROOT_URL_GetUsername + username, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                //callback.getUserProfile(response);
                try {
                    JSONObject respObj = new JSONObject(response);
                    String empId = respObj.getString("empId");
                    String username = respObj.getString("username");
                    String fName = respObj.getString("fName");
                    String email = respObj.getString("email");
                    callback.getUserProfile(empId, username, fName, email);
        ...
        queue.add(request);
        System.out.println("URL User : " + request);
    }

    public void postEmployeesTest(String username ,String password, final ICallBackUserLogin callback) {
        ...
        try {
            ...
            JsonObjectRequest stringRequest = new JsonObjectRequest(Request.Method.POST, URLs.ROOT_URL_Authentication, object2 ,new Response.Listener<JSONObject>() {
                ...
                public void onResponse(JSONObject response) {
                    callback.onListUsernameSuccess(response.toString());
    ...
    public void postEmployees(String username ,String password, final ICallBackUserToken callback) {
        try {
            ...
            StringRequest stringRequest = new StringRequest(Request.Method.POST, URLs.ROOT_URL_Authentication, new Response.Listener<String>() {
                ...
                public void onResponse(String response) {
                    try {
                        ...
                        String exp = respObj.getString("expired");
                        callback.onToken(username, fName,token, exp,true);
